...
阿里云支持可配置的审核场景，可以根据业务需求选择不同的检测类型组合：

#### 文本审核场景 (`aliyun_text_scenes`)
- `antispam` - 文本反垃圾（默认）
- `keyword` - 关键词检测
...
```json
{
  "aliyun_text_scenes": ["antispam", "keyword"]
}
```

#### 图片审核场景 (`aliyun_image_scenes`)
- `porn` - 智能鉴黄（默认）
- `terrorism` - 暴恐涉政（默认）
- `ad` - 广告识别（默认）
- `qrcode` - 二维码（默认）
...
```json
{
  "aliyun_image_scenes": ["porn", "terrorism", "ad", "sface"]
}
```

> [!TIP]
> 合理选择审核场景可以优化审核效率和成本。建议根据实际业务场景配置必要的检测类型。
...
### v2.1.0
- 新增阿里云审核场景可配置功能
- 文本审核支持选择 antispam（文本反垃圾）、keyword（关键词检测）场景
- 图片审核支持选择 porn（鉴黄）、terrorism（暴恐涉政）、ad（广告）、qrcode（二维码）、live（不良场景）、logo（Logo识别）、sface（敏感人脸）场景
- 支持自由组合审核场景，适应不同业务需求
